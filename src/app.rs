...
use crate::algorithms::{BinType, PackResult, pack_ga, pack_sa, pack_sorted, pack_svgnest};
use crate::steps::{Step, generate_steps};
use crate::types::{AlgoId, ComputeState, ShapeEntry};
use crate::ui::{
    ACC, BD, BG, ERR, INFO, OK, SF, SF2, TX, TX2, accent_btn, danger_btn, draw_board_canvas,
    draw_convergence_chart, field_row, gc, secondary_btn, section_header,
};
        ...
        thread::spawn(move || {
            ...
            let (result, hist) = match algo {
                AlgoId::MaxRects | AlgoId::Guillotine | AlgoId::BottomLeft | AlgoId::NfpGreedy => {
                    ...
                    for i in 0..5 {
                        tx.send(ProgressUpdate::Progress(i as f64 / 5.0, 0.0))
                            .unwrap_or(());
            ...
            // 发送完成消息
            tx.send(ProgressUpdate::Complete(result, hist, elapsed, bw, bh))
                .unwrap_or(());
        });
    }
                        ...
                        egui::Frame::none().inner_margin(margin).show(ui, |ui| {
                            ...
                            egui::Grid::new("board_cfg")
                                ...
                                .show(ui, |ui| {
                                    ...
                                    ui.end_row();
                                    ui.label(
                                        RichText::new("允许旋转").size(9.0).color(TX2).monospace(),
                                    );
                        ...
                        egui::Frame::none().inner_margin(margin).show(ui, |ui| {
                            ...
                            for algo in algos {
                                ...
                                let badge_color = match algo {
                                    AlgoId::MaxRects | AlgoId::Guillotine | AlgoId::BottomLeft => {
                                        OK
                                    }
                    ...
                    .show(ui, |ui| {
                        ...
                        if self.steps.is_empty() {
                            ...
                            ui.vertical_centered(|ui| {
                                ...
                                ui.add_space(10.0);
                                ui.label(
                                    RichText::new("选择算法，配置木板与图形")
                                        .size(11.0)
                                        .color(TX2)
                                        .monospace(),
                                );
                                ui.label(
                                    RichText::new("点击「开始优化计算」")
                                        .size(11.0)
                                        .color(TX2)
                                        .monospace(),
                                );
                        ...
                        for (bi, board) in boards.iter().enumerate() {
                            ...
                            if count == 0 && bi > step.board_idx {
                                continue;
                            }

                            let used: f64 =
                                board.placements.iter().take(count).map(|p| p.w * p.h).sum();
                            ...
                            egui::Frame::none()
                                .inner_margin(egui::Margin::symmetric(16.0, 0.0))
                                .show(ui, |ui| {
                                    egui::Frame::none().fill(SF).stroke(card_stroke).show(
                                        ui,
                                        |ui| {
                                            // Card header
                                            egui::Frame::none()
                                                ...
                                                .show(ui, |ui| {
                                                    ui.horizontal(|ui| {
                                                        ...
                                                        if count == board.placements.len()
                                                            && count > 0
                                                        {
                                                            let fw = bar_rect.width()
                                                                * (util / 100.0) as f32;
                                                            ui.painter().rect_filled(
                                                                egui::Rect::from_min_size(
                                                                    bar_rect.min,
